...
import { prioritizeTechnicianJobs } from "@/lib/domain/technician-job-priority";
import type {
  OperationsSnapshot,
  OrderStatus,
  ServiceOrder,
  ...
  return (
    <main className="min-h-dvh bg-[#f4f7f5] text-slate-900">
      <div className="grid min-h-dvh lg:grid-cols-[236px_minmax(0,1fr)]">
        ...
        <section className={`min-w-0 ${availableNav.length === 1 ? "pb-0" : "pb-20"} lg:pb-0`}>
          <header className="flex min-h-[72px] items-center justify-between border-b border-slate-200 bg-white/85 px-4 backdrop-blur sm:px-7">
            ...
            <div className="flex items-center gap-2"><div className="hidden text-right sm:block"><p className="text-xs font-bold capitalize text-slate-800">{role}</p><p className="text-[11px] text-slate-500">Mock assessment sign-in</p></div><button type="button" aria-label="Sign out" onClick={signOut} className={`inline-flex min-h-10 items-center gap-2 rounded-xl border border-slate-200 bg-white text-xs font-bold text-slate-700 transition hover:border-slate-300 hover:bg-slate-50 ${role === "technician" ? "w-10 justify-center px-0 sm:w-auto sm:px-3" : "px-3"}`}><LogOut className="size-3.5" /><span className={role === "technician" ? "hidden sm:inline" : undefined}>Sign out</span></button></div>
          </header>
      ...
      {availableNav.length > 1 && <nav className="fixed inset-x-0 bottom-0 z-20 grid grid-cols-3 border-t border-slate-200 bg-white px-1 py-1.5 shadow-[0_-10px_30px_rgba(15,23,42,0.08)] lg:hidden" aria-label="Mobile navigation">
...
function TechnicianJobs({ activeTechnician, technicians, onTechnicianChange, jobs, allOrders, completionOrderId, onCompletionOrderId, onStart, onComplete, completionFieldErrors, onCompletionFieldInput, selectedFiles, onFilesChange, pendingOrderActions, isCompleting }: { activeTechnician: OperationsSnapshot["technicians"][number] | undefined; technicians: OperationsSnapshot["technicians"]; onTechnicianChange: (technicianId: string) => void; jobs: ServiceOrder[]; allOrders: ServiceOrder[]; completionOrderId: string | null; onCompletionOrderId: (id: string | null) => void; onStart: (order: ServiceOrder) => void; onComplete: (event: FormEvent<HTMLFormElement>) => void; completionFieldErrors: FieldErrors; onCompletionFieldInput: () => void; selectedFiles: File[]; onFilesChange: (files: File[]) => void; pendingOrderActions: PendingOrderActions; isCompleting: boolean }) {
  ...
  const prioritizedJobs = prioritizeTechnicianJobs(jobs);
  return <div className="space-y-5">
    <div className="lg:hidden"><TechnicianMobileHome activeTechnician={activeTechnician} technicians={technicians} onTechnicianChange={onTechnicianChange} jobs={prioritizedJobs} onStart={onStart} onOpenCompletion={onCompletionOrderId} pendingOrderActions={pendingOrderActions} isCompleting={isCompleting} /></div>
...
function TechnicianMobileHome({ activeTechnician, technicians, onTechnicianChange, jobs, onStart, onOpenCompletion, pendingOrderActions, isCompleting }: { activeTechnician: OperationsSnapshot["technicians"][number] | undefined; technicians: OperationsSnapshot["technicians"]; onTechnicianChange: (technicianId: string) => void; jobs: ServiceOrder[]; onStart: (order: ServiceOrder) => void; onOpenCompletion: (id: string) => void; pendingOrderActions: PendingOrderActions; isCompleting: boolean }) {
  const currentJob = jobs[0];
  const remainingJobs = jobs.slice(1);

  return <section className="space-y-5">
    <header className="flex items-start justify-between gap-4">
      <div><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-teal-700">My jobs</p><h1 className="mt-1 text-2xl font-semibold tracking-tight text-slate-950">{jobs.length === 0 ? "No active jobs" : `${jobs.length} active ${jobs.length === 1 ? "job" : "jobs"}`}</h1></div>
      <label className="shrink-0"><span className="sr-only">Mock field user</span><select aria-label="Mock field user" value={activeTechnician?.id ?? ""} onChange={(event) => onTechnicianChange(event.target.value)} className="min-h-10 max-w-36 rounded-xl border border-amber-200 bg-amber-50 px-2.5 text-xs font-bold text-amber-950 outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-100">{technicians.map((technician) => <option key={technician.id} value={technician.id}>{technician.name} · {technician.code}</option>)}</select></label>
    </header>

    {!currentJob ? <div className="rounded-2xl border border-dashed border-slate-300 bg-white px-5 py-10 text-center"><CheckCircle2 className="mx-auto size-7 text-teal-600" /><h2 className="mt-3 font-semibold text-slate-950">You are clear for now</h2><p className="mt-1 text-sm text-slate-500">There are no assigned or in-progress jobs for {activeTechnician?.name ?? "this technician"}.</p></div> : <article className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="flex items-center justify-between gap-3 border-b border-slate-100 bg-[#f3f7f3] px-4 py-3"><p className="text-xs font-bold text-teal-800">Current job</p><StatusPill status={currentJob.status} /></div><div className="p-4"><p className="font-mono text-xs font-bold text-teal-700">{currentJob.orderNumber}</p><h2 className="mt-1 text-xl font-semibold tracking-tight text-slate-950">{currentJob.serviceType}</h2><p className="mt-2 text-sm font-medium text-slate-800">{currentJob.customerName}</p><a className="mt-1 inline-flex items-center gap-1.5 text-sm font-semibold text-teal-700 underline-offset-2 hover:underline" href={`tel:${currentJob.customerPhone}`}><Phone className="size-3.5" />{currentJob.customerPhone}</a><div className="mt-4 flex items-start gap-2.5 rounded-xl bg-slate-50 p-3"><MapPin className="mt-0.5 size-4 shrink-0 text-teal-700" /><p className="text-sm leading-5 text-slate-700">{currentJob.address}</p></div><div className="mt-4 border-l-2 border-[#dbf06d] pl-3"><p className="text-[10px] font-bold uppercase tracking-[0.12em] text-slate-500">Issue</p><p className="mt-1 text-sm leading-5 text-slate-700">{currentJob.issue}</p></div><p className="mt-4 text-xs font-medium text-slate-500">Scheduled {displayDate(currentJob.scheduledAt)}</p><div className="mt-5">{currentJob.status === "assigned" ? (() => { const feedback = getOrderActionFeedback({ orderId: currentJob.id, action: "start", fallbackLabel: "Start job", pending: pendingOrderActions }); return <button disabled={feedback.pending} aria-busy={feedback.pending || undefined} type="button" onClick={() => onStart(currentJob)} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#123b38] text-sm font-bold text-white shadow-sm disabled:cursor-wait disabled:opacity-70">{feedback.pending ? <LoaderCircle className="size-4 animate-spin" /> : <Wrench className="size-4" />}{feedback.label}</button>; })() : <button disabled={isCompleting} aria-busy={isCompleting || undefined} type="button" onClick={() => onOpenCompletion(currentJob.id)} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#dbf06d] text-sm font-bold text-[#123b38] shadow-sm disabled:cursor-wait disabled:opacity-70">{isCompleting ? <LoaderCircle className="size-4 animate-spin" /> : <CheckCircle2 className="size-4" />}{isCompleting ? "Completing…" : "Complete service"}</button>}</div></div></article>}

    {remainingJobs.length > 0 && <section aria-labelledby="up-next-heading" className="rounded-2xl border border-slate-200 bg-white"><div className="border-b border-slate-100 px-4 py-3"><h2 id="up-next-heading" className="text-sm font-semibold text-slate-950">Up next</h2></div><ul className="divide-y divide-slate-100">{remainingJobs.map((job) => <li key={job.id} className="px-4 py-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="font-mono text-xs font-bold text-teal-700">{job.orderNumber}</p><p className="mt-1 truncate text-sm font-semibold text-slate-900">{job.serviceType}</p><p className="mt-1 truncate text-xs text-slate-500">{job.address}</p></div><StatusPill status={job.status} /></div></li>)}</ul></section>}
  </section>;
}
