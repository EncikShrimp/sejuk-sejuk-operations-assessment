...
function OperationsAi({ question, state, onQuestionChange, onAsk }: { question: string; state: { loading: boolean; message: string | null; answer: string | null }; onQuestionChange: (value: string) => void; onAsk: (value: string) => void }) {
  const examples = ["What jobs did technician Ali complete last week?", "Which technician completed the most jobs this week?", "How many jobs were completed today?", "How many jobs were completed and what total amount was earned this week?"];
  return <div className="space-y-6"><SectionHeading eyebrow="Operations AI" title="Ask a bounded question about service data" description="The assistant can select only one read-only query. It does not receive database access, customer phone numbers, or address data." action={<span className="inline-flex items-center gap-2 self-start rounded-full border border-violet-200 bg-violet-50 px-3 py-1.5 text-xs font-bold text-violet-800"><ShieldCheck className="size-3.5" />Controlled retrieval</span>} /><div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_330px]"><section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><form onSubmit={(event) => { event.preventDefault(); onAsk(question); }}><label htmlFor="ai-question" className="text-sm font-semibold">Manager question</label><textarea id="ai-question" value={question} onChange={(event) => onQuestionChange(event.target.value)} maxLength={500} placeholder="Ask about completed jobs, amounts earned, or technician performance." className="mt-2 min-h-32 w-full rounded-xl border border-slate-300 bg-slate-50 p-3 text-sm leading-6 outline-none transition focus:border-teal-600 focus:ring-2 focus:ring-teal-100" /><div className="mt-3 flex items-center justify-between gap-3"><span className="text-xs text-slate-500">{question.length}/500</span><button disabled={state.loading || question.trim().length < 3} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-[#123b38] px-4 text-sm font-bold text-white disabled:opacity-50">{state.loading ? <LoaderCircle className="size-4 animate-spin" /> : <Send className="size-4" />}{state.loading ? "Retrieving…" : "Ask Operations AI"}</button></div></form>{(state.answer || state.message) && <div className={`mt-5 rounded-xl border p-4 ${state.answer ? "border-teal-200 bg-teal-50" : "border-amber-200 bg-amber-50"}`}><p className="text-[10px] font-bold uppercase tracking-[0.12em] text-slate-500">{state.answer ? "Verified response" : "Assistant limitation"}</p><p className="mt-2 text-sm leading-6 text-slate-800">{state.answer ?? state.message}</p></div>}</section><aside className="rounded-2xl bg-[#123b38] p-5 text-white"><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-[#dbf06d]">Supported queries</p><p className="mt-2 text-sm leading-6 text-teal-50/80">Use a suggested question to exercise a concrete, audited tool path.</p><div className="mt-5 space-y-2">{examples.map((example) => <button key={example} type="button" onClick={() => { onQuestionChange(example); onAsk(example); }} className="flex w-full items-start gap-2 rounded-xl border border-white/15 bg-white/5 p-3 text-left text-xs leading-5 text-white transition hover:bg-white/10"><ChevronRight className="mt-0.5 size-3.5 shrink-0 text-[#dbf06d]" />{example}</button>)}</div><div className="mt-6 border-t border-white/15 pt-4 text-xs leading-5 text-teal-50/65">Tool outputs are capped and server-validated. A model answer is only shown if its accompanying data exactly matches the tool result.</div></aside></div></div>;
}
